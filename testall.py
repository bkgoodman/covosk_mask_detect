...
def tagVideo(modelpath, videopath, outputPath=None):
    ...
    for frame in vreader(str(videopath)):
        ...
        #print ("FRAME")
        if (len(faces)>1): print (len(faces),"faces in",videopath)
        for face in faces:
            ...
            # draw prediction label
       	    if (len(faces)>1) and predicted==0: print ("Bad second face in ",videopath)
            if (areaperc > 0.75):
...
if __name__ == '__main__':
	...
	for x in glob.glob("/var/www/authbackend-ng/authlibs/logs/static/kioskimages/*.jpg"):
		if x.find("_ir") ==-1:
			try:
				r = tagVideo("/var/www/covosk-cv/covid-mask-detector/face_mask.ckpt",x)
				if r!=1: print (r,x)
			except:
				print ("BAD",x)
